...
import dk.exception.BaseException;
...
@Slf4j //lombok 定义的日志注解: 可以使用log变量, 来记录日志
@ControllerAdvice
public class ExceptionAdvisor {
     ...
     * try{
     *     controller.方法调用();
     * }catch(){ ===> @ExceptionHandler需要捕获的异常类型
     *
     * }
     * 日志的使用:
     * 日志的级别 由低到高排列: trace,debug,info,warn,error
     * 根据你配置的日志打印级别,调用的方法级别 >= 配置的级别, 才会打印
     * 比如如果是debug, 使用debug,info,warn,error才会打印错误日志
     */

    @ExceptionHandler(BaseException.class)
    @ResponseStatus(HttpStatus.OK)//响应状态码200
    @ResponseBody
    public Object handleBaseException(BaseException e){
        log.debug("================================");
        log.debug("服务端自定义异常", e); //打印错误日志+异常对象的堆栈信息
