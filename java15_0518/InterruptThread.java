...
//        t.interrupt();
        ...
        //（3）如果线程处于阻塞态，中断抛出InterruptedException时，会重置线程的中断标志
        Thread t=new Thread(()->{
            while (!Thread.interrupted()){ // 开始时isInterrupted=false;
                try {
                    System.out.println(Thread.currentThread().getName());
                    Thread.sleep(100000000);
                } catch (InterruptedException e) {
                    //线程阻塞的时候，抛出InterruptedException中断线程，同时isInterrupted=false;
                    e.printStackTrace();
                }
            }
        });
